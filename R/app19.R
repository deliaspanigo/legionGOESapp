  ...
  div(
    ...
    div(
      ...
      div(
        ...
        div(class = "sat-check",
            tags$input(type = "checkbox", id = "chk_Sentinel2A", checked = NA),
            span(class = "dot", style = "background:#ff4d4d;"),
            "Sentinel-2A"
        ),

        div(class = "sat-check",
            tags$input(type = "checkbox", id = "chk_Sentinel2B", checked = NA),
            span(class = "dot", style = "background:#ff9f43;"),
            "Sentinel-2B"
        ),

        div(class = "sat-check",
            tags$input(type = "checkbox", id = "chk_Landsat8", checked = NA),
            span(class = "dot", style = "background:#00d2d3;"),
            "Landsat 8"
        ),

        div(class = "sat-check",
            tags$input(type = "checkbox", id = "chk_Landsat9", checked = NA),
            span(class = "dot", style = "background:#54a0ff;"),
            "Landsat 9"
        ),

        div(class = "sat-check",
            tags$input(type = "checkbox", id = "chk_GOES16", checked = NA),
            span(class = "dot", style = "background:#a78bfa;"),
            "GOES-16"
        ),

        div(class = "sat-check",
            tags$input(type = "checkbox", id = "chk_GOES18", checked = NA),
            span(class = "dot", style = "background:#f472b6;"),
            "GOES-18"
        ),

        div(class = "sat-check",
            tags$input(type = "checkbox", id = "chk_GOES19", checked = NA),
    ...
    function createGridLines() {
      ...
      for (let lon = 0; lon < 180; lon += 30) {
        ...
        for (let lat = -90; lat <= 90; lat += 2) {
          const latRad = THREE.MathUtils.degToRad(lat);
          points.push(new THREE.Vector3(earthRadiusScene * Math.cos(latRad), earthRadiusScene * Math.sin(latRad), 0));
        }
