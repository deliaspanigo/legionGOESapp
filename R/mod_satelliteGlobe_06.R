  ...
  tagList(
      ...
      tags$script(HTML("
        (function() {
          ...
          function createSatelliteGlobe02(config) {
            ...
            const earthPolarRotationSpeed = 0.006;
            const cameraPolarMoveSpeed = 0.006;

            let cameraAzimuth = 0;
            let cameraElevation = 0;
            let cameraDistance = 7.0;

            const minCameraElevation = THREE.MathUtils.degToRad(-80);
            const maxCameraElevation = THREE.MathUtils.degToRad(80);
            ...
            function init() {
              ...
              /*
                OrbitControls queda sólo para zoom.
                La interacción de orientación la manejamos manualmente:
                - mouse horizontal: rotación sobre eje Norte-Sur.
                - mouse vertical: movimiento polar de la cámara.
              */
            ...
            function updateCameraPolarPosition() {
              const x =
                cameraDistance *
            ...
            function isNorthSouthRotationEnabled() {
              const input = byId(config.enable_ns_rotation_id);
              return input && input.checked;
            }

            function isPolarMovementEnabled() {
              const input = byId(config.enable_polar_movement_id);
              return input && input.checked;
            }

            function isAnyMouseInteractionEnabled() {
              return isNorthSouthRotationEnabled() || isPolarMovementEnabled();
            }

            function setupEarthPolarMouseRotation() {
              ...
              dom.addEventListener('pointerdown', function(event) {
                if (!isAnyMouseInteractionEnabled()) {
                  return;
                }
              ...
              dom.addEventListener('pointermove', function(event) {
                ...
                /*
                  Check 1:
                  Rotación sobre eje Norte-Sur terrestre.
                  Mouse izquierda/derecha.
                */
                if (isNorthSouthRotationEnabled()) {
                  earthGroup.rotation.y += dx * earthPolarRotationSpeed;
                }

                /*
                  Check 2:
                  Movimiento polar de la cámara.
                  Mouse arriba/abajo.
                */
                if (isPolarMovementEnabled()) {
                  cameraElevation += dy * cameraPolarMoveSpeed;

                  cameraElevation = Math.max(
                    minCameraElevation,
                    Math.min(maxCameraElevation, cameraElevation)
                  );
              ...
              dom.addEventListener('pointercancel', function(event) {
                isDraggingEarth = false;
            ...
            function setupResetButton() {
              ...
              resetButton.addEventListener('click', function() {
                /*
                  No movemos la cámara de golpe.
                  Sólo definimos los objetivos internos y dejamos que
                  updateResetAnimation() haga la transición suave.
                */
            ...
            function animate() {
              ...
              if (controls) {
                ...
                /*
                  Si el usuario usa zoom con rueda, OrbitControls cambia la distancia.
                  Guardamos esa nueva distancia para que el movimiento polar vertical
                  siga usando el zoom actual.
                */
    ...
    div(
      ...
      div(
        ...
        tags$label(
          tags$input(id = ns("enable_ns_rotation"), type = "checkbox", checked = "checked"),
          " Permitir rotación sobre eje Norte-Sur"
        ),

        tags$label(
          tags$input(id = ns("enable_polar_movement"), type = "checkbox", checked = "checked"),
          " Permitir movimiento polar"
        ),
        ...
        div(
          ...
          "Mouse horizontal: rota la Tierra sobre el eje Norte-Sur si el primer check está activo.",
          tags$br(),
          "Mouse vertical: mueve la vista hacia polo norte / polo sur si el segundo check está activo.",
          tags$br(),
          "El botón de reset vuelve suavemente a la posición inicial."
        )
      )
    )
  )
  ...
  moduleServer(id, function(input, output, session) {
    ...
    observe({
      ...
      config <- list(
        ...
        enable_ns_rotation_id = ns("enable_ns_rotation"),
        enable_polar_movement_id = ns("enable_polar_movement"),
