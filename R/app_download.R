# ===============================================================================
# GOES DOWNLOADER SHINY APP
# Etapas:
# 1) Satélite y fecha
# 2) Productos y tiempo
# 3) Verificación local/online + congelado de lista
# 4) Descarga efectiva en segundo proceso R
# ===============================================================================
...
get_default_download_dir <- function() {
  ...
  normalizePath(
    file.path(getwd(), "goes_downloads"),
    winslash = "/",
    mustWork = FALSE
  )
}
...
# ===============================================================================
# 2) HELPERS INVENTARIO LOCAL / ONLINE
# ===============================================================================
...
make_expected_inventory <- function(sat_date_config, products_time_config) {
  if (is.null(sat_date_config)) {
    stop("No hay configuración de satélite/fecha.")
  }

  if (is.null(products_time_config)) {
    stop("No hay configuración de productos/tiempo.")
  }
  ...
  div(
    ...
    div(
      ...
      div(
        ...
        tags$label("Hora UTC", `for` = ids$hour),
        selectInput(ids$hour, NULL, choices = hour_choices(), selected = "00", width = "100%")
      ),

      div(
        class = "compact-time-field",
        tags$label("Minuto UTC", `for` = ids$minute),
        selectInput(
          ids$minute,
          NULL,
          choices = minute_choices_for_product(product),
      ...
      div(
        class = "compact-time-field",
        tags$label("Segundo UTC", `for` = ids$second),
        selectInput(
          ids$second,
          NULL,
          choices = second_choices_for_product(product),
      ...
      div(
        ...
        uiOutput(paste0("estimated_files_", product))
      )
    ),
...
lock_switch_ui <- function(id, locked = FALSE) {
  initial_class <- if (isTRUE(locked)) "custom-lock-switch locked" else "custom-lock-switch unlocked"
  initial_icon <- if (isTRUE(locked)) "🔒" else "🔓"
  ...
  tags$button(
    ...
    tags$span(class = "custom-lock-label closed-label", "LOCK"),
    tags$span(class = "custom-lock-label open-label", "OPEN"),
    tags$span(class = "custom-lock-knob", initial_icon)
  )
}
...
ui <- fluidPage(
  ...
  tags$head(
    tags$style(HTML("
      ...
      .app-title { font-size: 28px; font-weight: 800; margin-top: 18px; margin-bottom: 4px; }
      .app-subtitle { color: #666; margin-bottom: 18px; }
      .card-box { background: white; border-radius: 14px; padding: 16px; margin-bottom: 16px; box-shadow: 0 2px 12px rgba(0,0,0,0.08); }
      .control-bar { background: #fff; border-radius: 14px; padding: 12px 16px; margin-bottom: 16px; box-shadow: 0 2px 12px rgba(0,0,0,0.08); display: flex; align-items: center; gap: 12px; }
      .custom-lock-switch { width: 92px; height: 44px; border-radius: 999px; border: none; padding: 4px; cursor: pointer; position: relative; transition: background 0.25s ease; box-shadow: inset 0 0 0 1px rgba(0,0,0,0.08); }
      .custom-lock-switch.unlocked { background: #74c0fc; }
      .custom-lock-switch.locked { background: #2f9e44; }
      .custom-lock-knob { width: 36px; height: 36px; border-radius: 50%; background: white; position: absolute; top: 4px; left: 4px; display: flex; align-items: center; justify-content: center; font-size: 20px; transition: left 0.25s ease; box-shadow: 0 2px 8px rgba(0,0,0,0.25); }
      .custom-lock-switch.locked .custom-lock-knob { left: 52px; }
      .custom-lock-label { font-size: 11px; font-weight: 700; color: white; position: absolute; top: 50%; transform: translateY(-50%); pointer-events: none; letter-spacing: 0.3px; }
      .custom-lock-label.open-label { right: 8px; }
      .custom-lock-label.closed-label { left: 8px; }
      .reset-button { font-size: 22px; min-width: 58px; height: 44px; border-radius: 12px; background: #fff4e6; border: 1px solid #ffc078; color: #e67700; }
      .control-text { font-size: 13px; color: #555; }
      .product-time-box { background: #fff; border: 1px solid #dee2e6; border-left: 6px solid #2c7be5; border-radius: 12px; padding: 8px 10px; margin-bottom: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.05); }
      .product-time-header { margin-bottom: 4px; }
      .product-title-line { display: flex; align-items: center; gap: 6px; font-size: 15px; line-height: 1.15; }
      .product-code-line { font-size: 12px; color: #555; font-weight: normal; }
      .product-time-controls-row { display: grid; grid-template-columns: 92px 92px 92px 1fr; gap: 8px; align-items: end; }
      .compact-time-field { margin: 0; padding: 0; }
      .compact-time-field label { font-size: 10px; margin-bottom: 1px; color: #555; font-weight: 600; }
      .compact-time-field .form-group { margin-bottom: 0 !important; }
      .compact-time-field select, .compact-time-field .selectize-input { min-height: 28px !important; height: 28px !important; padding: 2px 6px !important; font-size: 12px !important; line-height: 1.2 !important; }
      .compact-time-field .selectize-control { margin-bottom: 0 !important; }
      .estimated-files-inline { min-height: 28px; display: flex; align-items: end; }
      .estimated-files-inline .summary-box { width: 100%; padding: 5px 8px; border-radius: 7px; font-size: 12px; line-height: 1.2; margin: 0; white-space: normal; }
      .product-time-note { margin-top: 4px; font-size: 11px; color: #5c3d00; background: #fff4e6; border-left: 3px solid #f08c00; border-radius: 5px; padding: 4px 7px; }
      .summary-box { background: #eef6ff; border-left: 5px solid #2c7be5; padding: 12px; border-radius: 8px; font-family: Consolas, monospace; font-size: 13px; white-space: pre-wrap; }
      .locked-box { background: #f0fff4; border-left: 5px solid #2f9e44; padding: 12px; border-radius: 8px; font-family: Consolas, monospace; font-size: 13px; white-space: pre-wrap; }
      .warning-box { background: #fff4e6; border-left: 5px solid #f08c00; padding: 12px; border-radius: 8px; font-family: Consolas, monospace; font-size: 13px; white-space: pre-wrap; }
      .products-config-box { font-family: Consolas, monospace; font-size: 12px; line-height: 1.35; white-space: pre; overflow-x: auto; }
      .sat-date-inline-box { white-space: nowrap !important; overflow-x: auto; font-size: 12px; padding: 7px 10px; }
      .small-note { color: #555; font-size: 12px; }
      .download-log-box { background: #071018; color: #b8f7c1; border-radius: 10px; padding: 12px; height: 320px; overflow-y: auto; font-family: Consolas, monospace; font-size: 13px; white-space: pre-wrap; }
      .rules-table-wrap, .wide-table-wrap { width: 100%; overflow-x: auto; }
      .product-reference-table { width: 100%; font-size: 11px; margin-bottom: 0; }
      .product-reference-table th, .product-reference-table td { padding: 4px 5px !important; vertical-align: middle !important; }
      .product-reference-table .form-group, .product-reference-table .checkbox { margin: 0 !important; }
      .product-reference-table input[type='checkbox'] { margin: 0 !important; position: static !important; }
    ...
    tags$script(HTML("
      ...
      document.addEventListener('click', function(e) {
        ...
        Shiny.setInputValue(id + '_clicked', {locked: nextLocked, nonce: Math.random()}, {priority: 'event'});
      });
      Shiny.addCustomMessageHandler('set-lock-switch', function(message) {
        setLockSwitchState(message.id, message.locked);
      });
    "))
  ),

  div(class = "app-title", "LegionGOES Downloader"),
  div(class = "app-subtitle", "Satélite/fecha, productos/tiempo, verificación congelada y descarga en segundo proceso."),
  ...
  tabsetPanel(
    ...
    tabPanel(
      ...
      div(
        ...
        actionButton("reset_sat_date", "🔄", class = "reset-button"),
        div(class = "control-text", uiOutput("sat_date_control_text"))
      ),
      fluidRow(
        column(
          width = 4,
          div(class = "card-box", h4("Satélite"), selectInput("position", "Posición", choices = c("EAST", "WEST"), selected = "EAST")),
          div(
            class = "card-box",
            h4("Fecha"),
            numericInput("year", "Año", value = as.integer(format(Sys.Date(), "%Y")), min = 2017, max = 2100, step = 1),
            radioButtons("date_format", "Formato", choices = c("Gregoriano" = "gregorian", "Juliano" = "julian"), selected = "gregorian"),
            conditionalPanel(
              condition = "input.date_format == 'gregorian'",
              numericInput("month", "Mes", value = as.integer(format(Sys.Date(), "%m")), min = 1, max = 12, step = 1),
              numericInput("day", "Día", value = as.integer(format(Sys.Date(), "%d")), min = 1, max = 31, step = 1)
            ),
            conditionalPanel(
              condition = "input.date_format == 'julian'",
              numericInput("julian_day", "Día juliano", value = as.integer(format(Sys.Date(), "%j")), min = 1, max = 366, step = 1)
            )
          )
        ),
        column(width = 4, div(class = "card-box", h4("Estado satélite/fecha"), uiOutput("sat_date_state_box"))),
        column(width = 4, div(class = "card-box", h4("Configuración satélite/fecha"), uiOutput("sat_date_config_box")))
      )
    ),
    ...
    tabPanel(
      ...
      div(
        ...
        actionButton("reset_products_time", "🔄", class = "reset-button"),
        div(class = "control-text", uiOutput("products_time_control_text"))
      ),
      fluidRow(
        column(
          width = 6,
          div(class = "card-box", h4("Referencia de productos"), div(class = "rules-table-wrap", uiOutput("product_rules_ui"))),
          div(class = "card-box", h4("Configuración productos/tiempo"), uiOutput("products_time_config_box"))
        ),
        column(
          width = 6,
          div(
            class = "card-box",
            h4("Productos seleccionados"),
            uiOutput("product_tab_warning"),
            div(class = "small-note", "Los selectores de tiempo aparecen solo para los productos marcados en la tabla de referencia."),
            br(),
      ...
      fluidRow(column(width = 12, div(class = "card-box", h4("Estado productos/tiempo"), uiOutput("products_time_state_box"))))
    ),
    ...
    tabPanel(
      ...
      div(
        ...
        actionButton("reset_download_plan", "🔄", class = "reset-button"),
        div(class = "control-text", uiOutput("download_plan_control_text"))
      ),
      fluidRow(
        column(
          width = 4,
          div(
            class = "card-box",
            h4("Carpeta local de descargas"),
            div(class = "locked-box products-config-box", paste0("La app revisará automáticamente:\n", get_default_download_dir())),
            br(),
            actionButton("verify_local_online", "Verificar local y online", class = "btn btn-primary"),
            br(), br(),
            uiOutput("verification_summary")
          )
        ),
        column(
          width = 8,
          div(
            class = "card-box",
            h4("Detalle mínimo esperado"),
            div(class = "small-note", "Se muestra solo un ejemplo por producto. Las X indican posiciones no determinadas por la selección."),
            br(),
            div(class = "wide-table-wrap", tableOutput("expected_inventory_table"))
          )
        )
      ),
      fluidRow(
        column(
          ...
          div(
            class = "card-box",
            h4("Verificación local / online"),
            div(class = "small-note", "Esta tabla no descarga ni elimina archivos. Solo decide qué habría que hacer después."),
            br(),
            div(class = "wide-table-wrap", DTOutput("verification_table"))
          )
        )
      ),
      fluidRow(
        column(width = 4, div(class = "card-box", h4("Estado de lista congelada"), uiOutput("download_plan_state_box"))),
        column(width = 8, div(class = "card-box", h4("Resumen de lo que se descargará"), uiOutput("download_plan_summary")))
      ),
      fluidRow(
        column(
          width = 12,
          div(
            class = "card-box",
            h4("Lista congelada para descarga"),
            div(class = "small-note", "Esta lista sale exclusivamente de la verificación de esta pestaña. No se vuelve a consultar S3 al congelarla ni al descargar."),
            br(),
            DTOutput("download_plan_table")
          )
        )
      )
    ),
    ...
    tabPanel(
      ...
      fluidRow(
        column(
          ...
          div(
            class = "card-box",
            h4("Descarga efectiva"),
            uiOutput("download_ready_box"),
            br(),
            actionButton("start_download", "Download", class = "btn btn-success")
          )
        ),
        column(
          width = 8,
          div(
            class = "card-box",
            h4("Log de descarga"),
            div(class = "download-log-box", textOutput("download_log", container = span))
          )
        )
      ),
      fluidRow(column(width = 12, div(class = "card-box", h4("Estado de descarga"), DTOutput("download_status_table"))))
    )
  )
)
...
server <- function(input, output, session) {
  ...
  set_button_lock_state <- function(button_id, locked) {
    session$sendCustomMessage(
      type = "set-lock-switch",
      message = list(id = button_id, locked = isTRUE(locked))
    )
  }
  ...
  output$product_rules_ui <- renderUI({
    tags$table(
      class = "table table-sm table-bordered product-reference-table",
      tags$thead(tags$tr(tags$th("Usar"), tags$th("Producto"), tags$th("Código"), tags$th("Frecuencia"), tags$th("Minuto"), tags$th("Segundo"))),
      tags$tbody(
        lapply(seq_len(nrow(PRODUCT_SPECS)), function(i) {
          ...
          tags$tr(
            tags$td(checkboxInput(use_id, NULL, value = identical(product, "FDCF"), width = "24px")),
            tags$td(product),
            tags$td(PRODUCT_SPECS$product_code[i]),
            tags$td(PRODUCT_SPECS$frequency_label[i]),
            tags$td(minute_txt),
            tags$td(second_txt)
          )
        })
  ...
  for (product in PRODUCT_CHOICES) {
    local({
      ...
      output[[output_id]] <- renderUI({
        ...
        if (!use_product) {
          return(div(class = "summary-box", tags$strong("No seleccionado")))
        }
        ...
        div(
          class = "summary-box",
          tags$strong(paste0(n_files, " archivo", ifelse(n_files == 1, "", "s"))),
          tags$span(paste0(" | ", normalized$hour, ":", normalized$minute, ":", normalized$second, " UTC"))
        )
      })
    })
  }
  ...
  output$sat_date_control_text <- renderUI({
    if (isTRUE(rv$sat_date_locked)) {
      HTML("<b>🔒 Satélite/fecha cerrados.</b> Mueva el switch para editar. Presione 🔄 para resetear.")
    } else {
      HTML("<b>🔓 Satélite/fecha editables.</b> Mueva el switch para cerrar esta configuración.")
    }
  })

  output$sat_date_state_box <- renderUI({
    if (isTRUE(rv$sat_date_locked)) {
      div(class = "locked-box", "Satélite/fecha cerrados.\nLa app usará esta configuración para las siguientes pestañas.")
    } else {
      div(class = "warning-box", "Satélite/fecha editables.\nCierre esta sección antes de configurar productos.")
    }
  })

  output$sat_date_config_box <- renderUI({
    if (is.null(rv$sat_date_config)) {
      return(div(class = "warning-box", "No hay satélite/fecha cerrados."))
    }
  ...
  output$products_time_control_text <- renderUI({
    if (!isTRUE(rv$sat_date_locked)) {
      HTML("<b>Bloqueado.</b> Primero cierre satélite/fecha en la pestaña 1.")
    } else if (isTRUE(rv$products_time_locked)) {
      HTML("<b>🔒 Productos/tiempo cerrados.</b> Mueva el switch para editar. Presione 🔄 para resetear.")
    } else {
      HTML("<b>🔓 Productos/tiempo editables.</b> Mueva el switch para cerrar esta configuración.")
    }
  })

  output$product_tab_warning <- renderUI({
    if (!isTRUE(rv$sat_date_locked) || is.null(rv$sat_date_config)) {
      return(div(class = "warning-box", "Primero cierre satélite/fecha en la pestaña 1."))
    }
  ...
  output$products_time_state_box <- renderUI({
    if (!isTRUE(rv$sat_date_locked)) {
      return(div(class = "warning-box", "Bloqueado hasta cerrar satélite/fecha."))
    }

    if (isTRUE(rv$products_time_locked)) {
      div(class = "locked-box", "Productos/tiempo cerrados.")
    } else {
      div(class = "warning-box", "Productos/tiempo editables.")
    }
  })

  output$products_time_config_box <- renderUI({
    if (is.null(rv$products_time_config)) {
      return(div(class = "warning-box", "No hay productos/tiempo cerrados."))
    }
    ...
    product_time_text <- paste(
      vapply(
        ...
        function(x) {
          ...
          sprintf("%-7s | hora: %-5s | minuto: %-5s | segundo: %-5s | archivos: %s", x$product, x$hour, x$minute, x$second, n_files)
        },
        character(1)
      ),
      collapse = "\n"
  ...
  output$expected_inventory_table <- renderTable({
  ...
  output$verification_table <- DT::renderDT({
    ...
    if (is.null(x) || nrow(x) == 0) {
      return(DT::datatable(data.frame(Mensaje = "Todavía no se realizó la verificación."), rownames = FALSE, options = list(dom = "t", pageLength = 5)))
    }
  ...
  output$verification_summary <- renderUI({
    div(class = "locked-box products-config-box", rv$verification_summary)
  })

  output$download_plan_control_text <- renderUI({
    if (isTRUE(rv$download_plan_locked)) {
      HTML("<b>🔒 Verificación congelada.</b> La lista de descarga quedó cerrada en esta pestaña. Ahora puede ir a la pestaña 4 para descargar.")
    } else {
      HTML("<b>🔓 Verificación editable.</b> Primero ejecute la verificación local/online. Luego cierre este candado para congelar exactamente esa lista.")
    }
  })

  output$download_plan_state_box <- renderUI({
    if (isTRUE(rv$download_plan_locked)) {
      div(class = "locked-box", "Lista congelada.\nLa verificación local/online ya no puede repetirse hasta resetear o abrir el candado.\nPuede pasar a la pestaña 4 para descargar solo esta lista.")
    } else {
      div(class = "warning-box", "La lista todavía no está congelada.\nPrimero haga la verificación local/online en esta pestaña.\nLuego cierre el candado para habilitar la descarga segura en la pestaña 4.")
    }
  })

  output$download_plan_summary <- renderUI({
    div(class = "locked-box products-config-box", rv$download_plan_summary)
  })
  ...
  output$download_ready_box <- renderUI({
    if (!isTRUE(rv$download_plan_locked) || is.null(rv$download_plan)) {
      return(div(class = "warning-box", "Primero verifique y congele la lista en la pestaña 3."))
    }
  ...
  all_time_input_ids <- unlist(
    lapply(PRODUCT_CHOICES, function(product) unname(unlist(time_input_ids(product)))),
    use.names = FALSE
  )
  ...
  disable_products_time_inputs <- function() {
    for (id in c(product_check_ids, all_time_input_ids)) {
      shinyjs::disable(id)
    }
  }
  ...
  apply_product_specific_locks <- function(products = PRODUCT_CHOICES) {
    ...
    for (product in products) {
      ...
      if (identical(spec$minute_mode[[1]], "locked_first")) {
        updateSelectInput(session, ids$minute, choices = "FIRST", selected = "FIRST")
      ...
      if (identical(spec$second_mode[[1]], "locked_first")) {
        updateSelectInput(session, ids$second, choices = "FIRST", selected = "FIRST")
  ...
  enforce_all_first_rule <- function(product) {
    ...
    if (is.null(hour_value)) {
      return()
    }

    if (identical(hour_value, "FIRST")) {
      updateSelectInput(session, ids$minute, selected = "FIRST")
      updateSelectInput(session, ids$second, selected = "FIRST")
      return()
    }
    ...
    if (identical(hour_value, "ALL")) {
      if (identical(spec$minute_mode[[1]], "locked_first")) {
        ...
        updateSelectInput(session, ids$minute, selected = "ALL")
      }

      if (identical(spec$second_mode[[1]], "locked_first")) {
        updateSelectInput(session, ids$second, selected = "FIRST")
      } else {
        updateSelectInput(session, ids$second, selected = "ALL")
      }
  ...
  collect_product_times <- function(products) {
    out <- lapply(products, function(product) {
      ...
      if (is.null(hour) || !nzchar(hour)) {
        stop("Falta seleccionar hora para ", product)
      }

      if (is.null(minute) || !nzchar(minute)) {
        stop("Falta seleccionar minuto para ", product)
      }

      if (is.null(second) || !nzchar(second)) {
        stop("Falta seleccionar segundo para ", product)
      }
  ...
  reset_product_time_inputs <- function() {
    for (product in PRODUCT_CHOICES) {
      ids <- time_input_ids(product)
      updateSelectInput(session, ids$hour, choices = hour_choices(), selected = "00")
      updateSelectInput(session, ids$minute, choices = minute_choices_for_product(product), selected = default_minute_for_product(product))
      updateSelectInput(session, ids$second, choices = second_choices_for_product(product), selected = default_second_for_product(product))
    }
  }

  reset_product_checks <- function() {
    updateCheckboxInput(session, "use_LSTF", value = FALSE)
    updateCheckboxInput(session, "use_MCMIPF", value = FALSE)
    updateCheckboxInput(session, "use_FDCF", value = TRUE)
    updateCheckboxInput(session, "use_GLM", value = FALSE)
  }
  ...
  # -----------------------------------------------------------------------------
  # Lock / unlock satélite-fecha
  # -----------------------------------------------------------------------------

  lock_sat_date <- function() {
    date_info <- tryCatch(
      expr = {
        validate_date_inputs(input$year, input$date_format, input$month, input$day, input$julian_day)
      },
      error = function(e) {
    ...
    sat <- tryCatch(
      expr = {
        resolve_goes_satellite(input$position, date_info$gregorian_date)
      },
      error = function(e) {
        showNotification(conditionMessage(e), type = "error")
        NULL
    ...
    showNotification("Satélite/fecha cerrados. Ya puede configurar productos.", type = "message")
  ...
  unlock_sat_date <- function() {
    ...
    set_button_lock_state("toggle_sat_date", FALSE)
    showNotification("Satélite/fecha habilitados. Se borró la configuración de productos/tiempo.", type = "warning")
    TRUE
  }

  # -----------------------------------------------------------------------------
  # Lock / unlock productos-tiempo
  # -----------------------------------------------------------------------------

  lock_products_time <- function() {
    if (!isTRUE(rv$sat_date_locked) || is.null(rv$sat_date_config)) {
      showNotification("Primero debe cerrar satélite/fecha.", type = "warning")
      set_button_lock_state("toggle_products_time", FALSE)
      return(FALSE)
    }
    ...
    if (length(selected_products) == 0) {
      showNotification("Seleccione al menos un producto.", type = "error")
      set_button_lock_state("toggle_products_time", FALSE)
      return(FALSE)
    }
    ...
    product_times <- tryCatch(
      expr = {
        collect_product_times(selected_products)
      },
      error = function(e) {
        showNotification(conditionMessage(e), type = "error")
        NULL
    ...
    showNotification("Productos/tiempo cerrados correctamente.", type = "message")
  ...
  unlock_products_time <- function() {
    ...
    session$onFlushed(function() update_visible_product_boxes(), once = TRUE)
    set_button_lock_state("toggle_products_time", FALSE)
    showNotification("Productos/tiempo habilitados.", type = "warning")

    TRUE
  }

  # -----------------------------------------------------------------------------
  # Lock / unlock download plan
  # -----------------------------------------------------------------------------

  lock_download_plan <- function() {
    if (is.null(rv$verification_table) || nrow(rv$verification_table) == 0) {
      showNotification("Primero debe verificar local/online en esta pestaña.", type = "warning")
      set_button_lock_state("toggle_download_plan", FALSE)
      return(FALSE)
    }
    ...
    showNotification("Lista de descarga congelada.", type = "message")
  ...
  unlock_download_plan <- function() {
    clear_download_plan()
    showNotification("Lista de descarga descongelada/resetada.", type = "warning")
    TRUE
  }
  ...
  observeEvent(input$toggle_sat_date_clicked, {
    if (isTRUE(rv$sat_date_locked)) {
      unlock_sat_date()
    } else {
      lock_sat_date()
  ...
  observeEvent(input$reset_sat_date, {
    rv$sat_date_locked <- FALSE
    rv$sat_date_config <- NULL
    clear_products_time_config()
    enable_sat_date_inputs()
    ...
    updateSelectInput(session, "position", selected = "EAST")
    updateNumericInput(session, "year", value = as.integer(format(Sys.Date(), "%Y")))
    updateRadioButtons(session, "date_format", selected = "gregorian")
    updateNumericInput(session, "month", value = as.integer(format(Sys.Date(), "%m")))
    updateNumericInput(session, "day", value = as.integer(format(Sys.Date(), "%d")))
    updateNumericInput(session, "julian_day", value = as.integer(format(Sys.Date(), "%j")))

    showNotification("Satélite/fecha reseteados.", type = "message")
  })
  ...
  observeEvent(input$toggle_products_time_clicked, {
    if (!isTRUE(rv$sat_date_locked)) {
      showNotification("Primero debe cerrar satélite/fecha.", type = "warning")
      set_button_lock_state("toggle_products_time", FALSE)
      return()
  ...
  observeEvent(input$reset_products_time, {
    if (!isTRUE(rv$sat_date_locked)) {
      showNotification("Primero debe cerrar satélite/fecha.", type = "warning")
      set_button_lock_state("toggle_products_time", FALSE)
      return()
    ...
    showNotification("Productos/tiempo reseteados.", type = "message")
  })
  ...
  observeEvent(input$verify_local_online, {
    if (isTRUE(rv$download_plan_locked)) {
      showNotification("La verificación está congelada. Abra el candado o presione reset para volver a verificar.", type = "warning")
      return()
    }
    ...
    if (is.null(rv$products_time_config)) {
      showNotification("Primero debe cerrar productos/tiempo.", type = "warning")
      return()
    }
    ...
    expected <- tryCatch(
      expr = {
        make_expected_inventory(rv$sat_date_config, rv$products_time_config)
      },
      error = function(e) {
        showNotification(conditionMessage(e), type = "error")
        NULL
    ...
    if (is.null(expected) || nrow(expected) == 0) {
      showNotification("No se pudo generar el inventario esperado.", type = "error")
      return()
    }
    ...
    withProgress(message = "Verificando local y online", value = 0, {
      incProgress(0.15, detail = "Buscando archivos locales...")
      ...
      for (i in seq_len(nrow(product_prefixes))) {
        ...
        online_list[[i]] <- tryCatch(
          expr = {
            list_s3_prefix_paginated(product_prefixes$bucket[i], product_prefixes$day_prefix[i])
          },
          error = function(e) {
            showNotification(
      ...
      incProgress(0.10, detail = "Comparando local vs online...")
    })
    ...
    showNotification("Verificación local / online finalizada.", type = "message")
  })

  observeEvent(input$toggle_download_plan_clicked, {
    if (isTRUE(rv$download_plan_locked)) {
  ...
  observeEvent(input$reset_download_plan, {
    clear_download_plan()
    showNotification("Lista de descarga reseteada.", type = "message")
  })
  ...
  observeEvent(input$start_download, {
    if (isTRUE(rv$download_job_running)) {
      showNotification("Ya hay una descarga en proceso.", type = "warning")
      return()
    }

    if (!isTRUE(rv$download_plan_locked) || is.null(rv$download_plan)) {
      showNotification("Primero debe verificar y congelar la lista de descarga en la pestaña 3.", type = "warning")
      return()
    }
    ...
    if (nrow(plan) == 0) {
      showNotification("No hay archivos para descargar. La lista congelada está vacía.", type = "message")
    ...
    showNotification("Descarga iniciada en segundo proceso. Puede seguir usando la app.", type = "message")
  })
}
