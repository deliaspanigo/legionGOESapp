...
# Globo 3D interactivo con Sentinel y Landsat en tiempo real.
#
# Requisitos:
...
#
# Qué hace:
# - Descarga TLEs online desde CelesTrak.
# - Propaga órbitas con SGP4 usando satellite.js.
# - Usa la hora real actual del sistema en cada frame.
# - No usa tiempo acelerado.
# - La Tierra gira con velocidad sideral real.
# - Muestra latitud, longitud, altitud y radio orbital.
...
ui <- fluidPage(
  ...
  div(
    ...
    div(
      ...
      p("La posición se calcula usando la hora real actual, TLEs online de CelesTrak y propagación orbital SGP4 en JavaScript."),

      div(
        class = "control-row",
        tags$label("Modo"),
        div("Tiempo real orbital, sin aceleración.")
      ),
      ...
      div(
        class = "small-note",
        "Nota: el tamaño dibujado de los satélites está exagerado para que sean visibles. La distancia orbital sí se escala usando radio terrestre + altitud."
      )
    )
  ),
  ...
  tags$script(HTML("
    ...
    // TLEs de satélites de observación terrestre.
    ...
    async function loadTLEs() {
      ...
      try {
        ...
        if (satellitesData.length === 0) {
          setStatus('No se encontraron TLEs. Revisa conexión o el grupo de CelesTrak.');
        } else {
          setStatus(
    ...
    function addLandBlob(pointsLatLon, color) {
      ...
      pointsLatLon.forEach(function(p) {
        points.push(latLonToVector3(p[0], p[1], earthRadiusScene * 1.006));
      });
    ...
    function addContinentsApproximation() {
      ...
      addLandBlob([
        [70, -165], [60, -130], [50, -125], [35, -120],
        [20, -100], [10, -85], [25, -75], [45, -70],
        [60, -95], [70, -130]
      ], landColor);

      addLandBlob([
        [12, -80], [5, -75], [-10, -70], [-25, -65],
        [-45, -70], [-55, -60], [-35, -45], [-15, -35],
        [0, -50], [10, -65]
      ], landColor);

      addLandBlob([
        [70, -10], [60, 30], [55, 80], [45, 120],
        [25, 110], [10, 80], [20, 45], [35, 25],
        [45, 5], [60, -5]
      ], landColor);

      addLandBlob([
        [35, -15], [20, 5], [5, 20], [-15, 25],
        [-35, 18], [-35, 35], [-10, 45], [10, 40],
        [30, 30], [35, 5]
      ], landColor);

      addLandBlob([
        [-10, 110], [-20, 120], [-30, 135],
        [-38, 145], [-30, 155], [-18, 145]
      ], landColor);
    ...
    function createOrbitLineFromTLE(satrec, color) {
      ...
      // Aproximación visual de una órbita completa.
      // Para LEO, unos 100 minutos suelen cubrir una vuelta.
      for (let minutes = -50; minutes <= 60; minutes += 1) {
    ...
    function updateEarthRotation(date) {
      // Usamos tiempo sideral de Greenwich para orientar la Tierra.
      // La orientación visual de los continentes es aproximada, pero la velocidad es sideral real.
      const gmst = satellite.gstime(date);
