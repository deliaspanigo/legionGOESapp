...
ui <- fluidPage(
  ...
  div(
    ...
    div(
      ...
      div(
        class = "small-note",
        "La línea blanca atraviesa los polos y representa el eje de rotación terrestre. Las huellas de Sentinel/Landsat están aumentadas visualmente para poder verse sobre el globo; la distancia orbital y la posición sí se calculan desde TLE + SGP4."
      )
    )
  ),
  ...
  tags$script(HTML("
    ...
    function updateFootprint(sat, satWorldPosition) {
      ...
      const centerWorld = earthGroup.localToWorld(centerLocal.clone());

      conePoints.push(satWorldPosition.clone());
      conePoints.push(centerWorld.clone());

      const coneStep = sat.type === 'GEO' ? 8 : 4;

      for (let i = 0; i < ringLocalPoints.length; i += coneStep) {
        const ringWorld = earthGroup.localToWorld(
          ringLocalPoints[i].clone()
        );

        conePoints.push(satWorldPosition.clone());
        conePoints.push(ringWorld.clone());
      }

      const coneGeometry = new THREE.BufferGeometry().setFromPoints(conePoints);

      const coneMaterial = new THREE.LineBasicMaterial({
        color: sat.color,
        transparent: true,
        opacity: sat.type === 'GEO' ? 0.22 : 0.46
      });
    ...
    function updateVisibility() {
      satellitesData.forEach(function(sat) {
        ...
        if (sat.coneObject) {
          sat.coneObject.visible = visible;
        }

        if (sat.label && !visible) {
    ...
    function updateSatelliteLabel(sat) {
      ...
      const checkbox = document.getElementById(sat.checkboxId);
      const visibleSat = checkbox ? checkbox.checked : true;

      const showLabelsInput = document.getElementById('showLabels');
      const showLabels = showLabelsInput ? showLabelsInput.checked : true;

      if (!visibleSat || !showLabels) {
        sat.label.style.display = 'none';
        return;
      }
