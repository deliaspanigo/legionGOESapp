...
# Usa carpetas externas LEGION_* definidas en .Rprofile
# ==============================================================================
...
# ============================================================
# 0.1) CARPETAS LEGIONGOES
# ============================================================

LEGION_DATA_DIR <- Sys.getenv("LEGION_DATA_DIR")
LEGION_DOWNLOADS_DIR <- Sys.getenv("LEGION_DOWNLOADS_DIR")
LEGION_PROCESSED_DIR <- Sys.getenv("LEGION_PROCESSED_DIR")

if (!nzchar(LEGION_DATA_DIR) ||
    !nzchar(LEGION_DOWNLOADS_DIR) ||
    !nzchar(LEGION_PROCESSED_DIR)) {
  stop(
    "No están configuradas las carpetas LegionGOES.\n",
    "Revisá que el .Rprofile haya definido:\n",
    "LEGION_DATA_DIR, LEGION_DOWNLOADS_DIR y LEGION_PROCESSED_DIR."
  )
}

NC_DIR <- file.path(
  LEGION_DOWNLOADS_DIR,
  "MCMIPF"
)

TC_PROCESSED_DIR <- file.path(
  LEGION_PROCESSED_DIR,
  "MCMIPF",
  "truecolor"
)

dir.create(NC_DIR, recursive = TRUE, showWarnings = FALSE)
dir.create(TC_PROCESSED_DIR, recursive = TRUE, showWarnings = FALSE)

if (!dir.exists(NC_DIR)) {
  stop("No se pudo crear la carpeta de descarga: ", NC_DIR)
}

if (!dir.exists(TC_PROCESSED_DIR)) {
  stop("No se pudo crear la carpeta de procesados: ", TC_PROCESSED_DIR)
}

shiny::addResourcePath(
  prefix = "legion_data",
  directoryPath = LEGION_DATA_DIR
)

TC_ORIGINAL_PNG <- file.path(
  TC_PROCESSED_DIR,
  "truecolor_original_goes.png"
)

TC_WGS84_PNG <- file.path(
  TC_PROCESSED_DIR,
  "truecolor_global_wgs84.png"
)

TC_3857_PNG <- file.path(
  TC_PROCESSED_DIR,
  "truecolor_global_3857.png"
)

TC_ORIGINAL_URL <- "legion_data/processed/MCMIPF/truecolor/truecolor_original_goes.png"
TC_WGS84_URL    <- "legion_data/processed/MCMIPF/truecolor/truecolor_global_wgs84.png"
TC_3857_URL     <- "legion_data/processed/MCMIPF/truecolor/truecolor_global_3857.png"
...
message("LEGION_DATA_DIR: ", LEGION_DATA_DIR)
message("LEGION_DOWNLOADS_DIR: ", LEGION_DOWNLOADS_DIR)
message("LEGION_PROCESSED_DIR: ", LEGION_PROCESSED_DIR)
message("Carpeta de descarga MCMIPF: ", NC_DIR)
message("Carpeta de procesados True Color: ", TC_PROCESSED_DIR)
...
get_online_file_size <- function(bucket, key) {
  ...
  res <- tryCatch({
    ...
    warning("Error consultando HEAD: ", e$message)
    return(NULL)
  })
  ...
  suppressWarnings(as.numeric(size))
}
...
download_s3_object <- function(
  ...
  if (file.exists(dest)) {
    ...
    status_fun(paste("Archivo local existente:", basename(dest)))
    status_fun(paste("Peso local:", format_mb(local_size)))

    if (!is.na(online_size) &&
        !is.na(local_size) &&
        as.numeric(online_size) == as.numeric(local_size)) {
      status_fun("El archivo local coincide con el remoto. Se reutiliza.")
      return(dest)
    }

    if (is.na(online_size)) {
      status_fun("No se pudo verificar peso remoto. Se reutiliza el archivo local existente.")
      return(dest)
    }
  ...
  ok <- tryCatch({
    ...
    status_fun(paste("Error descargando archivo:", e$message))
    FALSE
  })
  ...
  if (!file.exists(dest)) {
    stop("No se pudo crear el archivo final: ", dest)
  }

  status_fun(paste("Archivo NetCDF listo en:", dest))
...
download_latest_mcmipf <- function(
  ...
  if (is.na(latest_path) || !file.exists(latest_path)) {
    stop("No se pudo descargar o reutilizar el último archivo MCMIPF.")
  }
...
rgb_rasters_to_png <- function(
  ...
  if (!file.exists(output_png)) {
    stop("No se pudo crear el PNG: ", output_png)
  }

  status_fun(paste("PNG creado:", output_png))
    ...
    tags$script(HTML("
      ...
      function tc_addOrUpdateOverlay(map, message) {
        ...
        overlay.on('error', function(e) {
          ...
          alert('No se pudo cargar la imagen True Color: ' + url);
        });
    ...
    div(
      ...
      div(
        ...
        actionButton(
          ...
          label = "Descargar/procesar última imagen True Color"
        ),
...
server <- function(input, output, session) {
  ...
  status_text <- reactiveVal(
    paste0(
      ...
      "NC_DIR:\n", NC_DIR, "\n\n",
      "TC_PROCESSED_DIR:\n", TC_PROCESSED_DIR, "\n\n",
      "Presioná el botón para descargar/procesar la última imagen True Color."
    )
  )
  ...
  selected_truecolor_url <- function(projection, processed) {
    ...
    if (projection == "goes") {
      return(paste0(TC_ORIGINAL_URL, "?v=", cache_buster))
    }

    if (projection == "wgs84") {
      return(paste0(TC_WGS84_URL, "?v=", cache_buster))
    }

    paste0(TC_3857_URL, "?v=", cache_buster)
  }

  selected_truecolor_path <- function(projection) {
    if (projection == "goes") {
      return(TC_ORIGINAL_PNG)
    }
  ...
  observeEvent(input$map_projection, {
    ...
    session$onFlushed(function() {
      ...
      png_to_show <- selected_truecolor_path(projection_now)

      if (!file.exists(png_to_show)) {
        showNotification(
          paste("No existe el PNG esperado:", basename(png_to_show)),
          type = "error",
          duration = 10
        )
        return()
  ...
  observeEvent(input$refresh_truecolor, {
    ...
    withProgress(message = "Procesando GOES True Color", value = 0, {
      ...
      progress_status(paste("Carpeta de descarga MCMIPF:", NC_DIR))
      progress_status(paste("Carpeta de procesados True Color:", TC_PROCESSED_DIR))
      ...
      info <- tryCatch({
        ...
        progress_status(paste("ERROR:", e$message))
      ...
      png_to_show <- selected_truecolor_path(projection)

      if (!file.exists(png_to_show)) {
        progress_status(paste("ERROR: no existe el PNG esperado:", png_to_show))
      ...
      progress_status(paste("PNG encontrado:", png_to_show))
      progress_status("Cargando PNG en Leaflet...")
